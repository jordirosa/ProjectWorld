#include "PWEquipmentShoulders.h"

void PWEquipmentShoulders::buildAssetPath(char* path)
{
	strcpy(path, "data/equip/Shoulders_");
	strcat(path, this->mAssetName);
}

PWEquipmentShoulders::PWEquipmentShoulders(SDLGraphics* graphics, const char* name) : PWEquipmentArmor(graphics, name)
{
	char path[100];
	std::string line;

	strcpy(this->mPartName, "Shoulders");
...
PWEquipmentShoulders::~PWEquipmentShoulders()
{
}
